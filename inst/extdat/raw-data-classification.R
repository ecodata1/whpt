...
year_report <- pivot_longer(class_report,names_to = c("year"),
                            cols = c(as.character(paste(2008:2018))))
year_report <- select(year_report, -ends_with("confidence"))
year_report$question <- "class"

confidence_report <- pivot_longer(class_report,
                                  names_to = "year",
                                  cols =
                                    as.character(paste0(
                                      2008:2018, "_confidence")))
confidence_report <- select(confidence_report,
                            -ends_with(as.character(paste0(2008:2018))))
confidence_report$question <- "confidence"
confidence_report$year <- gsub(pattern = "_confidence" ,
                               "",
                               confidence_report$year)
...
filtered_report <- pivot_wider(filtered_report,
                               names_from = question,
                               values_from = value)

filtered_report <- pivot_wider(filtered_report,
                               names_from = Parameter,
                               values_from = c(class, confidence))
